function [full_save_name, utility_user, user_per_k, utility_company, company_per_k]...
    = electricity_companyfirst_exponential_r_1(rho, psi_val, phi, lambda, g_def, toplot)
...
if toplot
    ...
    ax.FontSize = 18;
    ax.GridAlpha = 0.5;
    f6.Position = [100 100 750 400];
    xlabel('k');
    ylabel('Load (GW)');
end

% save data
save_name = strcat('companyfirst_lambda',strrep(num2str(lambda),'.','_'),'_psi_val_',strrep(num2str(psi_val),'.','_'),'_exp_r');
...
save(full_save_name, 'n', 'c', 'd', 'alpha','beta', 'lmin','lmax',...
    ...
    'SW','epsilon','mu','w','termTable','rk','lambda','psi_val','rho','phi','company_per_k','user_per_k')
...
if toplot
    ...
    savefig(f5, strcat(plot_path,"\",save_name, "_PricePlot"));
    savefig(f6, strcat(plot_path,"\",save_name, "_LoadPlot"));
end
