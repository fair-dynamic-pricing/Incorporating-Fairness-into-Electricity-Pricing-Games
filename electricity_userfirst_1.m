function [full_save_name, utility_user, user_per_k, utility_company, company_per_k] ...
    = electricity_userfirst_1(rho, psi_val, lambda, g_def, toplot)
...
l_star = [];
for i = 1:length(lm)
    l_star(i,1) = checklstar(w(i),d(i),beta(i),alpha(i),lambda,g_def);
end
...
if toplot
    ...
    % labels_load = {'Load' 'NominalDemand' 'l^*' 'LowerLimit' 'UpperLimit'};
...
% save data
save_name = strcat('userfirst_lambda',strrep(num2str(lambda),'.','_'),'_psi_val_',strrep(num2str(psi_val),'.','_'),'_linear_r');
...
save(full_save_name, 'n', 'c', 'd', 'alpha','beta', 'lmin','lmax',...
    ...
    'SW','epsilon','mu','w','termTable','rk','lambda','psi_val','rho','user_per_k', 'company_per_k')
...
if toplot
    ...
    savefig(f1, strcat(plot_path,"\",save_name, "_PricePlot"));
    savefig(f2, strcat(plot_path,"\",save_name, "_LoadPlot"));
end
...
function l_star = checklstar(w,d,beta,alpha,lambda,g_def)
syms l_comp
der    = derivativeu1_lk(l_comp,w,d,alpha,beta,lambda,g_def);
